            ...
            try:
                if response.status == 200 and "application/json" in response.headers.get("content-type", ""):
                    ...
                    if any(domain in url for domain in ["teams.microsoft.com", "substrate"]):
                        data = response.json()
                        # Optional debug line to see payload keys if calendar calls occur:
                        if "calendar" in url or "events" in url:
                            print("RAW CALENDAR PAYLOAD KEYS:", list(data.keys()) if isinstance(data, dict) else "List Payload")
                        parse_payload(data)
            except Exception:
                pass

        page.on("response", handle_response)
        print("🌐 Intercepting raw data feeds...")
